...
vim.keymap.set('n', '<leader>tt', ':ToggleTerm direction=horizontal<cr>', { desc = '[T]oggle [T]erminal' })
...
vim.keymap.set('n', '<leader>cp', function()
  local file = vim.fn.expand '%' -- Get the current file name
  if string.match(file, '%.py$') then -- Check if the file is a .go file
    local file_dir = vim.fn.expand '%:p:h' -- Get the directory of the current file
    -- local escaped_file = vim.fn.shellescape(file) -- Properly escape the file name for shell commands
    -- local command_to_run = "go run " .. escaped_file
    local command_to_run = 'py *.py'
    -- `-l 60` specifies the size of the tmux pane, in this case 60 columns
    local cmd = "silent !tmux split-window -h -l 60 'cd "
      .. file_dir
      .. ' && echo "'
    ...
    vim.cmd "echo 'Not a Py file.'" -- Notify the user if the file is not a Go file
  end
